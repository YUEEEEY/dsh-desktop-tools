...
import { existsSync, readFileSync, writeFileSync, openSync, statSync, readSync, closeSync, appendFileSync, mkdirSync, } from "node:fs";
...
function resolveRuntimePkg(pkg) {
    ...
    if (existsSync(join(hoisted, "package.json")))
        return hoisted;
    const nested = join(root, "node_modules", "@deepseek-ai", "dsh", "node_modules", ...seg);
...
function fetchLatestVersionAsync() {
    ...
    execFile(NPM_BIN, [
        ...
        "--fetch-retries=0",
        "--fetch-timeout=8000",
    ], { encoding: "utf8", windowsHide: true, timeout: 15000, shell: IS_WIN }, (error, stdout) => {
...
function latestVersion() {
    if (Date.now() - latestCache.at > 60000)
...
function isNewer(a, b) {
    ...
    const nums = (v) => (v || "0").split(".").map((n) => Number.parseInt(n, 10) || 0);
...
function runtimeStatus() {
    ...
    const updateAvailable = installed !== undefined && latest !== undefined && isNewer(latest, installed);
    return {
...
const SUBPROCESS_OLD_THROW = "\tthrow new Error(`subprocess-local: terminal inspection is unsupported on platform ${platform}`);";
const SUBPROCESS_NEW_THROW = '\tif (platform === "win32") return new WindowsProcessInspector(internals);\n' +
...
function patchSubprocessWin32() {
    if (!IS_WIN)
        return { ok: true, detail: "非 Windows 平台，无需补丁" };
    const st = subprocessCheck();
    if (st.ok)
        return st;
    if (!st.file || !st.content)
        return st;
    ...
    const withClass = st.content.replace("function createProcessInspector", `${WINDOWS_INSPECTOR_SOURCE}\nfunction createProcessInspector`);
...
function minimalPresetCheck() {
    ...
    const file = join(root, "node_modules", "@deepseek-ai", "dsh", "config", "agent-presets", "minimal", "agent.cordis.yml");
    try {
        const content = readFileSync(file, "utf8");
...
function fetchBalance(cb) {
    ...
    if (!key)
        return cb({ error: "未找到 DEEPSEEK_API_KEY" });
    execFile("curl.exe", [
        "-s",
        "--max-time",
        "15",
    ...
    ], { encoding: "utf8", windowsHide: true, timeout: 20000 }, (error, stdout) => {
        if (error)
            return cb({ error: String(error?.message ?? error) });
        try {
...
function fetchUsage() {
    try {
        ...
        const sessions = (j.tables?.sessions ?? {});
...
function billingPayload(prices, cb) {
    ...
    fetchBalance((balance) => {
        if ("error" in u)
            return cb({ balance, usage: u });
        const est = (u.input / 1e6) * prices.input +
...
function startRuntimeUpdate() {
    const root = runtimeRoot();
    if (!root)
        return { ok: false, error: "未定位到 dsh 安装目录" };
    if (updateChild !== null) {
...
async function startDesktop() {
    ...
    if (process.env.DSH_HOME)
        args.push("--home", process.env.DSH_HOME);
    try {
...
function apply(ctx, config) {
    ...
    if (config.checkUpdatesOnLaunch) {
        setTimeout(() => {
            ...
            if (installed && latest && isNewer(latest, installed)) {
                console.log(`[desktop-tools] 检测到 dsh 新版本 ${latest}（当前 ${installed}），可在 /panel 面板点击“更新运行时”升级`);
            }
        }, 1500);
    }
    ...
    if (cmdline.includes("--no-desktop"))
        wantDesktop = false;
    if (cmdline.includes("--desktop"))
        wantDesktop = true;
    if (wantDesktop) {
        ...
        setTimeout(() => {
            const bin = desktopHostPath();
            if (!bin || !existsSync(bin)) {
                console.log("[desktop-tools] 已请求打开桌面端，但未找到宿主二进制（dsh-desktop.exe）——请重新安装插件或 cargo build");
                return;
            }
    ...
    ctx.effect(() => ctx.webServer.register({
        kind: "exact",
        path: "/api/billing",
        handler: async (_req, res) => billingPayload(prices, (payload) => json(res, payload)),
