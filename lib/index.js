...
function fetchLatestVersionAsync() {
  ...
  execFile(
    ...
    [
      "/C",
      "npm view @deepseek-ai/dsh version --no-audit --no-fund --fetch-retries=0 --fetch-timeout=8000",
    ],
...
function billingPayload(prices, cb) {
  ...
  fetchBalance((balance) => {
    if (u.error) return cb({ balance, usage: u });
    const est =
...
/** 定位 Rust 桌面宿主（dsh-desktop.exe）：
 *  1) Config.desktopBin  2) 环境变量 DSH_DESKTOP_BIN
 *  3) 插件内嵌 <插件包>/desktop/win32-x64/  4) 仓库内构建 <dsh-env>/host/target/release/
 *  5) PATH */
function desktopHostPath() {
  ...
  const cands = [
    ...
    join(pluginRoot(), "desktop", "win32-x64", exeName),
...
async function desktopStatus() {
  ...
  return {
    ...
    hint: exists
      ? "宿主就绪，可打开桌面端"
      : "未找到宿主二进制（请重新安装插件，或在 dsh-env/desktop 运行 cargo build --release，或设置 desktopBin/DSH_DESKTOP_BIN）",
  };
}
