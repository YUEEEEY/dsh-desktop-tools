...
主窗口内打开（`Ctrl+Shift+E` / 托盘 / 菜单），用于直接阅读与修改工作区文件：

- 左侧文件树（默认跳过 `.git` / `node_modules` 等）、右侧编辑器，按扩展名识别语言；
- `Ctrl+S` 保存到磁盘，`Ctrl+Shift+B` 或页面内"← 返回主界面"回到主界面；
- 编辑器内核为 Monaco（按需加载，离线时自动降级为内置编辑器）；
